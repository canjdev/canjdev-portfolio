...
function autoBind(instance: object): void {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof instanceRecord[key] === "function") {
      instanceRecord[key] = (
        instanceRecord[key] as (...args: unknown[]) => unknown
      ).bind(instance);
    }
  });
}
...
class App {
  ...
  onWheel(e: Event) {
    ...
    const delta =
      wheelEvent.deltaY ||
      (wheelEvent as { wheelDelta?: number }).wheelDelta ||
      (wheelEvent as { detail?: number }).detail ||
      0;
